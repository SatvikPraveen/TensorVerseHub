...
def create_prediction_chart(
    predictions: np.ndarray, class_names: Optional[List[str]] = None, top_k: int = 5
):
...
def main():
    ...
    app_type = st.sidebar.selectbox(
        "Application Type", ["Image Classification", "Text Classification", "Model Analysis"]
    )
                        ...
                        if demo_mode:
                            ...
                            if model is not None:
                                ...
                                predictions, inference_time = run_inference(
                                    model, processed_image, model_type
                                )
                ...
                with st.spinner("Processing text..."):
                    if demo_mode:
                        ...
                        if model is not None:
                            ...
                            # Run inference
                            predictions, inference_time = run_inference(
                                model, processed_text, model_type
                            )
