...
class TensorFlowModelServer:
    ...
    def preprocess_image(
        self, image_data: str, target_size: Tuple[int, int] = (224, 224)
    ) -> np.ndarray:
        """
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        avg_inference_time = (
            self.total_inference_time / self.inference_count if self.inference_count > 0 else 0
        )
        ...
        status.update(
            {
                ...
                "average_inference_time_ms": (
                    (model_server.total_inference_time / model_server.inference_count * 1000)
                    if model_server.inference_count > 0
                    else 0
                ),
            }
        )
...
def predict_image():
    ...
    try:
            ...
            top_predictions = [
                {
                    ...
                    "class_name": (
                        model_server.model_metadata.get("class_names", [str(idx)])[idx]
                        if "class_names" in model_server.model_metadata
                        else str(idx)
                    ),
                }
        ...
        return jsonify(
            {"success": True, "predictions": top_predictions, "raw_predictions": predictions_list}
        )
    ...
    try:
        ...
        for i, input_item in enumerate(inputs):
            try:
                ...
                batch_predictions.append({"success": True, "predictions": predictions.tolist()})

            except Exception as e:
                batch_predictions.append({"error": str(e)})

        return jsonify({"success": True, "batch_size": len(inputs), "results": batch_predictions})
