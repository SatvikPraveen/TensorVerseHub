...
class PruningDemo:
    ...
    def train_baseline_model(
        ...
        callbacks = [
            ...
            tf.keras.callbacks.ReduceLROnPlateau(factor=0.2, patience=2, monitor="val_loss"),
        ]
    ...
    def visualize_pruning_results(self, sparsity_analyses: Dict, benchmark_results: Dict):
        ...
        if benchmark_results:
            ...
            params = [
                benchmark_results[model]["parameters"] / 1000 for model in models_bench
            ]  # in thousands

            bars = ax2.bar(
                models_bench, params, color=["blue", "orange", "green"][: len(models_bench)]
            )
        ...
        if benchmark_results:
            ...
            bars = ax3.bar(
                models_bench, times, color=["blue", "orange", "green"][: len(models_bench)]
            )
            ...
            for model in models:
                ...
                for layer in layer_names:
                    if layer in sparsity_analyses[model]["layer_sparsity"]:
                        model_sparsity.append(
                            sparsity_analyses[model]["layer_sparsity"][layer]["sparsity_ratio"]
                        )
                    else:
    ...
    def run_complete_demo(
        ...
        pruned_model = self.demonstrate_magnitude_pruning(
            self.original_model, train_ds, val_ds, target_sparsity
        )
