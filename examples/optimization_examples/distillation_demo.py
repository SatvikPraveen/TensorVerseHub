...
class DistillationDemo:
    ...
    def evaluate_models(
        self, models: Dict[str, tf.keras.Model], test_ds: tf.data.Dataset
    ) -> Dict[str, Dict]:
    ...
    def benchmark_inference(
        self, models: Dict[str, tf.keras.Model], test_input: np.ndarray
    ) -> Dict[str, Dict]:
    ...
    def visualize_results(
        self, evaluation_results: Dict, benchmark_results: Dict, training_histories: Dict
    ):
        ...
        for i, model in enumerate(models):
            ax5.scatter(sizes[i], accuracies[i], s=100, color=colors[i], label=model, alpha=0.7)
            ax5.annotate(
                model, (sizes[i], accuracies[i]), xytext=(5, 5), textcoords="offset points"
            )
    ...
    def print_summary(self, evaluation_results: Dict):
        ...
        for model, results in evaluation_results.items():
            compression = (
                f"{teacher_params / results['parameters']:.1f}x" if model != "teacher" else "1.0x"
            )
            print(
                f"{model:<20} {results['test_accuracy']:<10.3f} "
...
def main():
    ...
    try:
        demo.run_complete_demo(
            input_shape, args.num_classes, args.teacher_epochs, args.student_epochs
        )
