...
class TFRecordHandler:
    ...
    def serialize_image_example(
        self, image_path: str, label: int, additional_features: Optional[Dict] = None
    ) -> bytes:
        """
    ...
    def serialize_text_example(
        self, text: str, label: int, additional_features: Optional[Dict] = None
    ) -> bytes:
        """
...
class DataPipeline:
    ...
    def create_image_dataset(
        ...
        if augment:
            dataset = dataset.map(
                self._apply_image_augmentation, num_parallel_calls=tf.data.AUTOTUNE
            )
    ...
    def create_text_dataset(
        self, texts: List[str], labels: List[int], max_length: int = 128, vocab_size: int = 10000
    ) -> Tuple[tf.data.Dataset, tf.keras.layers.TextVectorization]:
        """
    ...
    def create_mixed_precision_dataset(self, dataset: tf.data.Dataset) -> tf.data.Dataset:
        ...
        return dataset.map(cast_to_mixed_precision, num_parallel_calls=tf.data.AUTOTUNE)
...
class DataAugmentation:
    ...
    def mixup(dataset: tf.data.Dataset, alpha: float = 0.2) -> tf.data.Dataset:
        ...
        def mixup_fn(batch_x, batch_y):
            ...
            mixed_y = tf.squeeze(lam[:, 0, 0, 0:1]) * tf.cast(batch_y, tf.float32) + (
                1 - tf.squeeze(lam[:, 0, 0, 0:1])
            ) * tf.cast(y_shuffled, tf.float32)
    ...
    def cutmix(dataset: tf.data.Dataset, alpha: float = 1.0) -> tf.data.Dataset:
        ...
        def cutmix_fn(batch_x, batch_y):
            ...
            lam_adjusted = 1.0 - (
                tf.cast((x2 - x1) * (y2 - y1), tf.float32) / tf.cast(image_h * image_w, tf.float32)
            )

            mixed_y = lam_adjusted * tf.cast(batch_y, tf.float32) + (1 - lam_adjusted) * tf.cast(
                y_shuffled, tf.float32
            )
