...
pytestmark = pytest.mark.skipif(
    not NBCONVERT_AVAILABLE, reason="nbformat/nbconvert not available for notebook testing"
)
    ...
    def execute_notebook(self, notebook_path: str, working_dir: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            preprocessor = ExecutePreprocessor(
                timeout=self.timeout, kernel_name=self.kernel_name, allow_errors=False
            )
            ...
            error_msg = str(e)
            self.executed_notebooks.append(
                {"path": notebook_path, "success": False, "error": error_msg}
            )
        ...
        try:
            ...
            for cell in notebook.cells:
                if cell.cell_type == "markdown":
                    ...
                    if any(
                        obj_word in source for obj_word in ["learning objectives", "objectives"]
                    ):
...
class TestNotebookFoundations:
    ...
    def test_notebook_01_tensors_operations(self, notebook_tester, notebooks_dir):
        ...
        notebook_path = os.path.join(
            notebooks_dir, "01_tensorflow_foundations", "01_tensors_operations_execution.ipynb"
        )
        ...
        assert (
            outputs.get("errors", []) == []
        ), f"Should have no errors, but found: {outputs.get('errors')}"

    @pytest.mark.parametrize(
        "notebook_name", ["02_data_pipelines_tfrecords.ipynb", "03_debugging_profiling.ipynb"]
    )
    def test_foundation_notebooks_exist(self, notebook_name, notebooks_dir):
        ...
        notebook_path = os.path.join(notebooks_dir, "01_tensorflow_foundations", notebook_name)
...
class TestNotebookStructure:
    ...
    def test_notebook_content_standards(self):
        ...
        for notebook_path in notebook_files:
            ...
            if issues:
                issues_found.append({"notebook": notebook_path.name, "issues": issues})
                            ...
                            if line.startswith("import ") or line.startswith("from "):
                                ...
                                if "import *" in line:
                                    import_issues.append(
                                        f"{notebook_path.name}: Wildcard import found"
                                    )

                                # Check for relative imports without proper structure
                                if line.startswith("from .") or line.startswith("from .."):
                                    import_issues.append(
                                        f"{notebook_path.name}: Relative import found"
                                    )
...
def run_notebook_tests(
    notebooks_dir: str, output_report: str = "notebook_test_report.html"
) -> bool:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", "-o", default="notebook_test_report.html", help="Output path for test report"
    )
    parser.add_argument(
        "--timeout", "-t", type=int, default=300, help="Timeout per notebook cell in seconds"
    )
